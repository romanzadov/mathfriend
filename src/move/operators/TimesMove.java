...
public class TimesMove {
	...
	public Image overEqualsMoves(Image im,
		...
		ArrayList <Integer> key = TermMath.findTreePositionOfSelected(im.tr, sel);
		Term ghostsel = TermMath.findTermUsingKey(second, key);
	...
	public static Term removeChild(Term main, int indexOfChild){

		//check inputs to see that we're getting a term with multiplication
		Operator op = main.operator;
		if(op instanceof Times  &&  main.getChilds().size()>2){}
		else{ System.out.println("remove of multiplication run on a term without multiplication. In TimesMove. ");  return null; }
		...
		if(indexOfChild == 0){
			...
			//next,remove the times that's after it
			result.getChilds().remove(0);
		}

		else{
			//remove the operator in front of term, and then the term
			result.getChilds().remove(indexOfChild -1 );
			result.getChilds().remove(indexOfChild -1 );
	...
	public static Term replaceMultiplicativeTerm(Term main, Term multiple, int positionOfReplacement){
		

		//check inputs
		Operator op = main.operator;
		if(op instanceof Times  &&  main.getChilds().size()>0){}
		else{ System.out.println("replace multiplication run on a term without multiplication. In TimesMove. ");  return null; }
		...
		if(result instanceof simpleterm  ){
		...
		else if(main.toString().equals("("+result.toString()+")")){
			result = multiple;
		}
			...
			result.getChilds().set(positionOfReplacement, multiple);
		}
